...
class FastLip(FastSpeech2):
    ...
    def forward(self, src_tokens, vid2ph=None, guide_face=None,
        ...
        if hparams['use_pitch_embed']:
            decoder_inp = decoder_inp + self.add_pitch(decoder_inp_origin, f0, uv, vid2ph, ret)
        # add energy embed
        ...
        if hparams['use_ref_enc']:
            ...
            decoder_inp = decoder_inp + ref_embed

        # add guide face
        guide_face = guide_face.permute(0, 3, 1, 2)   # [B, h, w, c] -> [B, c, h, w]
        # print('guide_face shape: ', guide_face.shape)
        ...
        if hparams['imgdecoder_2D']:
            ...
            #     concat_z = torch.cat([guide_face_embed[:, 0, :], x[:, frame_idx, :]], dim=1)   # [B, hidden_size]
            #     output_frames.append(self.img_decoder(concat_z))
            # output_frames = torch.stack(output_frames, dim=1)   # [B, seq_len, hidden_size]
