...
class TransTTSDataset(BaseDataset):
    ...
    def collater(self, samples):
        ...
        tgt_lengths, sort_order = tgt_lengths.sort(descending=True)
...
class TransformerTtsTask(BaseTask):
    ...
    def validation_step(self, sample, batch_idx):
        ...
        attn_ks = src_lengths.float() / tgt_lengths.float()
    ...
    def infer_batch(self, sample):
        ...
        if not hparams['profile_infer']:
            src_lengths = sample['src_lengths'] - 1  # exclude eos
            tgt_lengths = (1.0 - hit_eos[:, 1:].float()).sum(dim=-1) + 1
            src_padding_mask = input.eq(0) | input.eq(self.eos_idx)  # also exclude eos
            src_seg_mask = input.eq(self.seg_idx)
            ...
            phone_coverage_rate = utils.get_phone_coverage_rate(encdec_attn, src_padding_mask, src_seg_mask,
                                                                target_padding_mask)
            attn_ks = src_lengths.float() / tgt_lengths.float()
            diagonal_focus_rate, diag_mask = utils.get_diagonal_focus_rate(encdec_attn, attn_ks, tgt_lengths,
                                                                           src_padding_mask,
                                                                           target_padding_mask)
        else:
